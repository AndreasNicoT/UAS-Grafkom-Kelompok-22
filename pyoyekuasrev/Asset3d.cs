...
namespace ProyekUTS
{
    class Asset3d
    {
        ...
        Matrix4 _view;
        Matrix4 _projection;
        Matrix4 model = Matrix4.Identity;   // Model Matrix      ==> Matrix ini yang akan berubah saat terjadi transformasi
        ...
        public void loadwithnormal(int sizeX, int sizeY)
        {
            _vertexBufferObject = GL.GenBuffer();
            GL.BindBuffer(BufferTarget.ArrayBuffer, _vertexBufferObject);
            ...
            _shader = new Shader(path + "objectcolor.vert", path + "objectcolor.frag");
            _shader.Use();
        ...
        public void setFragVariabel(Vector3 objectColor)
        {
            _shader.SetVector3("objColor", objectColor);

            foreach (var item in child)
            {
                item.setFragVariabel(objectColor);
            }
        }
